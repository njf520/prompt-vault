...
│                   FastAPI Service                          │
│                                                            │
│  POST /api/prompts          → Create prompt (v1)          │
│  PUT  /api/prompts/{name}   → Commit new version          │
│  GET  /api/prompts/{name}   → Get latest (or pinned ver.) │
│  GET  /api/prompts/{name}/history  → Full version list    │
│  GET  /api/prompts/{name}/diff?v1=1&v2=2 → Unified diff  │
│  GET  /api/tags             → All tags across library     │
│  DELETE /api/prompts/{name} → Delete prompt + history     │
└───────────────────────┬────────────────────────────────────┘
                        │
┌───────────────────────▼────────────────────────────────────┐
│               JSON File Store (vault.json)                 │
│         (swappable for SQLite, Postgres, etc.)             │
└────────────────────────────────────────────────────────────┘
```
...
**Example: Fetch a specific version by hash (for reproducibility)**
```bash
...
system_prompt = get_prompt("support-greeting", version=3)
```
